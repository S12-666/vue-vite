...
const handleExport = () => {
    ...
    // 1. 定义导出的表头和对应的数据字段 (根据你的 el-table-column 重新提取)
    const headers = {
        upid: 'upid',
        slabid: 'slabid',
        thick: 'tgThick',
        ...
        fm_start_temp: 'FMTemp',
        tg_temp: 'tgTemp',
        r_start_time: 'RollingStartTime',
        r_end_time: 'RollingEndTime',
        steelspec: 'steelSpec',
        label: 'FaultStatus' // 如果你也想把底层的故障标签数据导出来，可以带上这个字段
    };
    ...
    // 3. 拼接数据行 (处理 null/undefined 并在字段外加双引号防止内容包含逗号导致错位)
    ...
    // 生成带时间戳的文件名 (这里改成了 轧制报表)
    const timestamp = new Date().toISOString().replace(/[-:T]/g, '').slice(0, 14);
    link.setAttribute('href', url);
    link.setAttribute('download', `轧制报表_${timestamp}.csv`);
